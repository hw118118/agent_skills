...
2. **npm** — each plugin also publishes as an npm package so it can be installed with `npm install -g <package-name>` (or `npx`). Include the necessary `package.json` inside the plugin directory and wire up a `postinstall` / CLI entry so the plugin files land in the user's Claude Code plugins directory.
